...
class TableViewController: UITableViewController {
    var jobs :[String] = ["job1", "job2", "job3", "job4"]
    
    
    class CustomerPendingCell: UITableViewCell {
    ...
    override func tableView(_ tableView: UITableView,
       ...
       let cell = tableView.dequeueReusableCell(withIdentifier: "test",
                             for: indexPath) as! CustomerPendingCell

       // Fetch the data for the row.
       let theFood = jobs[indexPath.row]
            
       // Configure the cell’s contents with data from the fetched object.
       cell.jobOutlet?.text = theFood
